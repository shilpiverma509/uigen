...
function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  if (toolName === "str_replace_editor") {
    ...
    if (command === "create") return file ? `Creating ${file}` : "Creating file";
    if (command === "str_replace" || command === "insert") return file ? `Editing ${file}` : "Editing file";
    if (command === "view") return file ? `Reading ${file}` : "Reading file";
  }
  if (toolName === "file_manager") {
    const action = args?.action;
    const file = typeof args?.path === "string" ? args.path.split("/").pop() : null;
    if (action === "rename") return file ? `Renaming ${file}` : "Renaming file";
    if (action === "delete") return file ? `Deleting ${file}` : "Deleting file";
  }
  return toolName;
}
